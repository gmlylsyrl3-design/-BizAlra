...
const buildTextRequest = (messages: any[]) => {
  ...
  if (openaiApiKey) {
    return {
      ...
      body: JSON.stringify({ model: "gpt-3.5-turbo", messages, temperature: 0.8, max_tokens: 350 }),
    };
  }
...
const requestTextCompletion = async (messages: any[]) => {
  const requestConfig = buildTextRequest(messages);
  if (!requestConfig) {
